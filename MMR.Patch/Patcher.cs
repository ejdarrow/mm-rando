...
namespace MMR.Patch
{
    public static class Patcher
    {
        ...
        public static byte[] ApplyPatchEncrypted(string filePath, FileTable fileTable, byte[] key, byte[]? iv)
        {
            using var outStream = File.OpenRead(filePath);
            return ApplyPatchEncrypted(outStream, fileTable, key, iv);
        }
        ...
        /// <param name="fileTable"></param>
        /// <returns><see cref="SHA256"/> hash of the patch.</returns>
        public static byte[] ApplyPatchEncrypted(Stream inStream, FileTable fileTable, byte[] key, byte[]? iv)
        {
            var aes = Aes.Create();
        ...
        /// <param name="fileTable"></param>
        /// <returns><see cref="SHA256"/> hash of the patch.</returns>
        public static byte[] CreatePatchEncrypted(string filePath, FileTable fileTable, byte[] key, byte[]? iv)
        {
            using var outStream = File.Open(filePath, FileMode.Create);
            return CreatePatchEncrypted(outStream, fileTable, key, iv);
        }
        ...
        /// <param name="fileTable"></param>
        /// <returns></returns>
        public static byte[] CreatePatchEncrypted(Stream outStream, FileTable fileTable, byte[] key, byte[]? iv)
        {
            var aes = Aes.Create();
